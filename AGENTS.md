...
This repository is currently in Phase 0 and contains planning and architecture documentation only. Core plans live in `docs/`, including the product build plan, SDLC plan, workflows, risk register, and requirements traceability matrix. Approved decisions live in `docs/adr/`; use `docs/adr/template.md` and update `docs/adr/README.md` when adding one.

The planned implementation is a modular monorepo: Python domain code under `rekanvault/`, FastAPI and worker entrypoints under `apps/api/` and `apps/worker/`, the Next.js UI under `apps/web/`, shared schemas under `packages/contracts/`, migrations under `migrations/`, and categorized suites under `tests/`. Do not create these directories ahead of the phase that introduces them.

## Build, Test, and Development Commands

There is no executable application or committed build configuration yet. For documentation changes, use lightweight checks such as:

- `rg --files docs` — list the current documentation set.
- `rg "RV-DEC|P[0-9]+-GATE" docs` — check decision and phase-gate references.
- `git diff --check` — detect whitespace errors before review (when working in a full Git checkout).

Phase 1 will introduce reproducible `uv` and `pnpm` scripts. Once present, treat committed scripts and lockfiles as authoritative; do not invent parallel command wrappers.

## Coding Style & Naming Conventions

Keep Markdown concise, use descriptive headings, fenced code blocks, and repository-relative paths. Preserve established product terms such as `RekanVault`, `RV-DEC-####`, and `P#-GATE`. ADR files follow `RekanVault_ADR_<Phase>_<4-digit-number>_<short-title>.md`, with a lowercase hyphenated title.

Planned Python code targets 3.12 and will use Ruff formatting/linting plus mypy. Planned TypeScript uses ESLint and the TypeScript compiler. Follow four-space indentation in Python and existing formatter output elsewhere.

## Testing Guidelines

Every requirement must map to an observable acceptance condition or phase gate. Planned backend tests use pytest (including Hypothesis and testcontainers); frontend tests use Vitest, Testing Library, and Playwright. Name tests by behavior, keep permission and lifecycle negative cases beside happy paths, and retain gate evidence under the planned `docs/release-evidence/` structure.

## Commit & Pull Request Guidelines

Git history is unavailable in this workspace snapshot, so no local commit-message pattern can be verified. Use short, imperative subjects with a focused scope. Branches follow `feat/<scope>`, `fix/<scope>`, or `chore/<scope>`, and merges are squashed. Every pull request must link its phase task and acceptance condition, describe validation performed, identify contract/security/migration effects, and include screenshots for visible UI changes. Never commit secrets, credentials, sensitive fixtures, or unredacted logs.
...
- ponytail `.codex/plugins/cache/ponytail` is mandatory skill for all work.
