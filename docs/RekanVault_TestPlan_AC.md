...
**Status: Implemented (2026-08-07).** All 13 P4 to-dos checked off. 57 new tests. Full Postgres + Qdrant pipeline verified against 61/63 Google Drive docs (587 vectors). Golden set: 180 questions across 10 categories. P4-GATE partially met — dense-only recall at 42.8% (target 90%); full hybrid pipeline built but not runtime-verified due to Qdrant free-tier limits. Two known non-architectural gaps documented in SDLC §9 exit gate.
...
| `P4-T5` | Exact phrase, Indonesian semantic, English semantic, mixed-language, acronym, and entity queries. | Golden question set queries reach Recall@10 ≥ 0.90, MRR ≥ 0.85, nDCG@10 ≥ 0.88, and 100% citation resolution accuracy. | Test line + Product Plan §25 | ⚠️ Partial — dense-only: R@10=0.43, MRR=0.30, nDCG=0.33. Full hybrid pipeline (lexical+RRF+reranker) built, not runtime-verified. 2 email dumps not indexed (GPU/ONNX needed). |
